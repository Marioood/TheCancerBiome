...
{ 
  [AutoloadBossHead]
	public class PrimaryClone : ModNPC
	{
    ...
    public const float ExpertShootTime = (1.5f * 60);
    ...
    public override void AI()
    {
      ...
      if(AiState == StateWait) {
        ...
        if(Main.expertMode) {
          int fireTimer = (int)(AiTeleTimer % ExpertShootTime);
          
          if(fireTimer == 0 && offTile) {
            if(Main.netMode != NetmodeID.MultiplayerClient) {
              double ang = NPC.AngleTo(thePlayer.Center);
        ...
        if(modularTimer == 0) {
          ...
          if(offTile && NPC.Distance(thePlayer.Center) < 16 * 32) {
            //shoot projectiles
            if(choice > 0.75f) {
              //ranges from 4 to 20 projectiles depending on current health
              AiTeleTimer = ((int)(NPC.lifeMax - NPC.life) / 1000 + 1) * 4 * 10 * (Main.expertMode ? 2 : 1);
              AiState = StateFire;
      ...
      float maxLen = Main.expertMode ? 7 : 6;
      if(len > maxLen) {
        NPC.velocity /= len;
        NPC.velocity *= maxLen;
      }
    ...
    public bool isOffTile() {
      int r = 2;
      
      for(int y = -r; y <= r; y++) {
        for(int x = -r; x <= r; x++) {
          Tile cur = Framing.GetTileSafely((int)NPC.Center.X / 16 + x, (int)NPC.Center.Y / 16 + y);
          if(cur.HasTile && !Main.tileFrameImportant[cur.TileType]) //frame important tiles are almost always not solid
            return false;
        }
      }
      return true;
